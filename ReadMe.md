...
- 問題/懸念点
    > - [x] [懸念事項] 検索処理の上限問題
    >   グラフDBに対して検索処理を実行した結果、取得件数が多いとエラーになる可能性がある。以下のような対応策が考えられるが有効ではない為未作成となっています。
    >   - 検索処理で返却する出力件数の制限。
    ...
    >
    > - [x] [問題] グラフDBのバージョン問題
    >   グラフDBのプロシージャを実行していますが、Neo4jとプロシージャ用のパッケージ(APOCに関するJarファイル)のバージョン情報を一致していないとプロシージャを使用することができない可能性があります。
    >   グラフDBのバージョンアップ時、プロシージャも別途更新する必要があります。GitHubよりNeo4jのバージョンと一致するバージョンをダウンロードすることができます。ただし、プロシージャはバージョンアップ時に使用できなくなることもあるため、ドキュメントの確認は必須事項です。
    >
